...
function gridlistSetSelectionMode(element, mode)
	...
	GUIinfo[gridlist].selectionMode = mode
	...
	--	if not element then break end
		...
		if info.visible and ( not parentInfo or parentInfo.visible ) then
			...
			elseif info.guiType == "editbox" then
				...
				text = getTextBeingDrawnIntoEdit(text,info.scroll, scrollEnd or #text, info.width-4)
			...
			elseif info.guiType == 'gridlist' then
				...
				dxDrawRectangle(x,y+21,info.width,info.height-21, info.color)
				...
				for row=1,#rows do
					...
					if info.selectedRow == row and not info.selectedColumn  then 
						dxDrawRectangle(x, y+1+((row)*20), info.width, 20, tocolor(50,50,100,200))
					end
					...
					for i=1,#columns do	
						...
						if rows[row][i] then
							...
							if info.selectedRow == row and info.selectedColumn == i then
								...
								dxDrawRectangle(columnX, columnY+1+((row)*20), columnWidth, 20, tocolor(50,50,100,200))
								
							end
							local rowColor = rows[row][i].textColor or defaultTextColor
							dxDrawText(rows[row][i][1],columnX+5, columnY+21+((row-1)*20), columnX+columnWidth, 20+columnY+21+((row-1)*20),rowColor, defaultTextScale*0.95,"default",'left','center',false,false)
