...
function createWindow(x,y,width,height,relative,text,parent,color)
	if x and y and width and height then
		...
		GUIinfo[element] = { guiType = "window", x = x,y = y, width = width,height = height,text = text or "",color = color, visible = true, resource = sourceResourceRoot }
		return element
...
function createCheckBox(x,y,width,height,relative,text,parent,color,state)
	...
	if x and y and width and height and text then
		...
		local element = addElement()
		if parent then setElementParent(element,parent) end
		GUIinfo[element] = { guiType = "checkbox", x = x,y = y, width = width,height = height,text = text,color = color, visible = true, resource = sourceResourceRoot, isChecked = state }
		return element
...
function createGridList(x,y,width,height,relative,parent,color)
	...
	if x and y and width and height then
		...
		GUIinfo[element] = { guiType = "gridlist", x = x,y = y, width = width,height = height,color = color, visible = true, resource = sourceResourceRoot, scroll = 0, columns = {}, rows = {} }
		return element
...
function createEditBox(x,y,width,height,relative,text,parent,color,clearOnFirstClick,masked,maskedAfterClick)
	if x and y and width and height and text then
		...
		clearOnClick = clearOnFirstClick, masked = masked, maskedAfterClick = maskedAfterClick, caretIndex = 0 }
		return element
...
function createLabel(x,y,width,height,relative,text,parent,color)
	if x and y and width and height and text then
		...
		local element = addElement()
		if parent then setElementParent(element,parent) end
		GUIinfo[element] = { guiType = "label", x = x,y = y, width = width,height = height,text = text,color = color, visible = true, resource = sourceResourceRoot }
		return element
...
function createButton(x,y,width,height,relative,text,parent,color)
	if x and y and width and height and text then
		...
		local element = addElement()
		if isElement(parent) then setElementParent(element,parent) end
		GUIinfo[element] = { guiType = "button", x = x,y = y, width = width,height = height,text = text,color = color, visible = true, resource = sourceResourceRoot }
		return element
