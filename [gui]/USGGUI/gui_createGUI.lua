...
function getSkinColor(key)
    if(currentSkin and currentSkin.defaultColors and currentSkin.defaultColors[key]) then
        return currentSkin.defaultColors[key]
    else
        return false
    end
end
...
function createScrollArea(x,y,width,height,relative,parent, noScrollBar)
    if x and y and width and height then        
        ...
        if not color then color = getSkinColor("scrollarea") or tocolor(0,0,0,130) end
...
function createWindow(x,y,width,height,relative,text,parent,color)
    if x and y and width and height then        
        ...
        if not color then color = getSkinColor("window") or tocolor(0,0,0,190) end
        GUIinfo[element] = { guiType = "window", x = x,y = y, width = width,height = height,text = text or "",color = color, visible = true, 
        resource = sourceResourceRoot, titleBarColor = getSkinColor("window_title_bar") or tocolor(255,0,0), textColor = tocolor(0,0,0,200),ignoreOnRender=ignoreOnRender, movable = true }
...
function createCheckBox(x,y,width,height,relative,text,parent,color,state)
    if x and y and width and height and text then
        local element, x,y,width,height,ignoreOnRender = initGUI(x,y,width,height,relative,parent)
        if not color then color = getSkinColor("checkbox") or tocolor(0,0,0,190) end
...
function createRadioButton(x,y,width,height,relative,text,parent,color,state)
    if x and y and width and height and text then
        local element, x,y,width,height,ignoreOnRender = initGUI(x,y,width,height,relative,parent)
        if not color then color = getSkinColor("radiobutton") or tocolor(0,0,0,190) end
...
function createGridList(x,y,width,height,relative,parent,color)
    if x and y and width and height then
        local element, x,y,width,height,ignoreOnRender = initGUI(x,y,width,height,relative,parent)
        if not color then color = getSkinColor("gridlist") or tocolor(0,0,0,160) end
        GUIinfo[element] = { guiType = "gridlist", x = x,y = y, width = width,height = height,color = color, visible = true, resource = sourceResourceRoot, 
        contentRenderTarget = dxCreateRenderTarget(width,height-22,true), columns = {}, rows = {}, displayedRows = {}, rowCount = 0, scrollPixels=0, selectionMode=0,ignoreOnRender=ignoreOnRender
        , scrollBarProperties = {}}
        return element
    end
end
...
function gridlistAddRow(gridlist)
    if isElement(gridlist) and GUIinfo[gridlist] then
        ...
        table.insert(GUIinfo[gridlist].displayedRows, GUIinfo[gridlist].rowCount)
        for i=1,#GUIinfo[gridlist].columns do
...
function createImage(x,y,width,height,relative,img,parent,color)
    if x and y and width and height then
        ...
        ignoreOnRender = ignoreOnRender, img = img, color = color or tocolor(255,255,255) }
        return element
    end
end
...
function createEditBox(x,y,width,height,relative,text,parent,color,clearOnFirstClick,masked,maskedAfterClick)
    if x and y and width and height and text then
        local element, x,y,width,height,ignoreOnRender = initGUI(x,y,width,height,relative,parent)
        if not color then color = getSkinColor("editbox") or tocolor(255,255,255) end
...
function createMemo(x,y,width,height,relative,text,parent,color)
    if x and y and width and height and text then
        local element, x,y,width,height,ignoreOnRender = initGUI(x,y,width,height,relative,parent)
        if not color then color = getSkinColor("memo") or tocolor(255,255,255) end
...
function createLabel(x,y,width,height,relative,text,parent,color)
    if x and y and width and height and text then
        local element, x,y,width,height,ignoreOnRender = initGUI(x,y,width,height,relative,parent)
        if not color then color = getSkinColor("text") or tocolor(255,255,255) end
...
function createButton(x,y,width,height,relative,text,parent,color)
    if x and y and width and height and text then
        local element, x,y,width,height,ignoreOnRender = initGUI(x,y,width,height,relative,parent)
        if not color then color = getSkinColor("button") or tocolor(175,75,0,210) end
...
function createTabPanel(x,y,width,height,relative,parent,color)
    if x and y and width and height then
        local element, x,y,width,height,ignoreOnRender = initGUI(x,y,width,height,relative,parent)
        if not color then color = getSkinColor("tabpanel") or tocolor(0,0,0,190) end
