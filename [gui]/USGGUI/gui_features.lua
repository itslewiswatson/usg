local keyTable = { -- values: key with shift enabled, true -> key:lower()
...
["["]='{', ["]"]='}', [";"]=':', [","]='<', ["-"]='_', ["."]='>',["/"]='?', ["\\"]='|', ["="]='+' }
-- special keys their input string representation
...
addEvent("onUSGGUIAccept",true)
addEvent("onUSGGUISClick",true)
addEvent("onUSGGUIChange",true)
function onClientCharacter(key)
	local info = GUIinfo[elementInFocus]
	if not ( info and isElement(elementInFocus) ) or ( info.guiType ~= "editbox" and info.guiType ~= "memo" ) or ( info.readOnly ) then return true end
...
function onClientKey(orgKey,pressed)
		...
		if orgKey == "mouse_wheel_up" or orgKey == "mouse_wheel_down" then		
			...
			if ( maxScroll and maxScroll > 0) then
				if orgKey == "mouse_wheel_down" then
					info.scrollPixels = math.floor(math.min(maxScroll,info.scrollPixels+(0.08*maxScroll)))
				elseif orgKey == "mouse_wheel_up" then				
					info.scrollPixels = math.floor(math.max(0,info.scrollPixels-(0.08*(maxScroll))))
				end
			end
		end
	end
	if ( info.guiType == "editbox" or info.guiType == "memo" ) then
		...
		elseif not info.readOnly and ( orgKey == 'backspace' or orgKey == 'delete' ) then		
			...
			triggerEvent("onUSGGUIChange",elementInFocus, oldText, info.text)
		elseif orgKey == "enter" and not info.readOnly then
			if ( info.guiType == "memo" ) then
				onClientCharacter(string.char(10))
			else
				triggerEvent("onUSGGUIAccept",elementInFocus,info.text)
			end
		else return
		end
...
function isChildInFocus(element)
	if ( isElement(element) and GUIinfo[element] and elementInFocusParents[element] or element == elementInFocus ) then
		return true
	end
	return false
end
		...
		if ( cx > elementStartX and cx < elementEndX ) and ( cy > elementStartY and cy < elementEndY ) then
			...
			for i=1,#children do
				...
				if ( isElement(cclicked) ) then
					...
					elementInFocusParents[element] = true
				end
			end
		end
	end
...
function onClientClick(button,state, sx, sy)
	...
	if isElement(element) then
		...
		elseif info.guiType == "window" and section == 'title' and button == 1 and info.movable then
			if state == 'down' then		
				mouseXdragOffset, mouseYdragOffset = sx-info.x,sy-info.y	
				if not GUIElementBeingHeld then			
					GUIElementBeingHeld = element
		...
		elseif info.guiType == "editbox" or info.guiType == "memo" then 		
			...
			local relX, relY = sx-elX, sy-elY
			if(info.guiType == "editbox") then
				local text = getTextBeingDrawnIntoEdit(info.masked and string.rep("*",#info.text) or info.text,info.scroll, #info.text, info.width-8)
				local charI = info.scroll
				local totWidth = 0
				for char in text:gmatch(".") do
					totWidth = totWidth+dxGetTextWidth(char)
					if(totWidth > relX) then
						break
					end
					charI = charI+1
				end
				-- if the difference with last character and click position is less than X pixels, move the cursor
				if(relX-totWidth < 25) then
					info.caretIndex = charI
				end
			elseif(info.guiType == "memo") then
				local caret = 0
				for i, line in ipairs(info.lines) do
					if(relY > (i-1)*15 and relY < (i)*15) then
						local totWidth = 0
						...
						-- if the difference with last character and click position is less than X pixels, move the cursor
						if(relX-totWidth < 50) then
							info.caretIndex = caret
						end
						break
					end
					caret = caret+#line+1
		...
		elseif info.guiType == "gridlist" then 		
			if state == 'down' then
				...
				if ( loc and height and loc+elY <= sy and loc+elY+height >= sy and sx > elX+info.width-12 ) then
					...
					if(y < -2) then -- column area
						local width = 0
						local columnClicked
						for column=1,#info.columns do
							local columnX, columnY, columnWidth = width, 0, (info.columns[column].width*info.width)
						...
						if(columnClicked) then
							if(info.sortColumn == columnClicked) then
								info.sortAscending = not info.sortAscending
							else
								info.sortColumn = columnClicked
								info.sortAscending = false
							end
							sortGridList(element) -- resort
						end
					elseif(y >= 0) then -- row area
						...
						for i=rowIndex,info.rowCount do
							local row = rows[i]
		...
		elseif info.guiType == "radiobutton" and state == "down" then
			if(not info.selected) then -- select a new one, clearing the old selection if existing
		...
		if ( button == 1 and state == "up" and isElement(element) ) then
			triggerEvent( "onUSGGUISClick", element, section, sx, sy )
		end
	end
...
function manageMouseOnRender()
	...
	if(not isElement(elementScrollHeld.element)) then elementScrollHeld.element = nil end
	...
	if ( isElement(elementScrollHeld.element) ) then
