...
function createDialog(title,text,dialogType, ...)
	...
	if ( type(text) == "string" and type(dialogType) == "string" ) then
		...
		if ( dialogType == "confirm" ) then
			...
			focus(dialog.window)
		elseif ( dialogType == "input" or dialogType == "password" ) then
			dialog.window = createWindow('center','center',400,150,false,"USG ~ "..title)
			createLabel('center','top',380,25,false,text,dialog.window)
			dialog.input = createEditBox('center',50,380,25,false,"",dialog.window)
			if(dialogType == "password") then
				exports.USGGUI:setProperty(dialog.input, "masked", true)
			end
			dialog.OK = createButton(250,100,75,25,false,"OK",dialog.window)
			dialog.cancel = createButton(50,100,75,25,false,"Cancel",dialog.window)
			dialog.match = args[1]
			dialog.minimumLength = args[2] or 0
			focus(dialog.input)
		...
		for window, dialog in pairs(dialogs) do
			if ( source == dialog.OK or source == dialog.cancel ) then -- this is the dialog
				...
				if ( dialog.dialogType == "confirm" ) then
					...
					closeDialog(window)
				elseif ( dialog.dialogType == "input" or dialog.dialogType == "password" ) then
					if ( dialog.OK == source ) then
						local input = getText(dialog.input)
						if ( not dialog.match or string.match(input,dialog.match) ) then
							if(#input >= dialog.minimumLength) then
								triggerEvent("onUSGGUIDialogFinish", window, dialog.match and input:match(dialog.match) or input)
							else
								close = false
								exports.USGmsg:msg("Your input must be at least "..dialog.minimumLength.." characters long!",255,0,0)
							end
						else
							close = false
