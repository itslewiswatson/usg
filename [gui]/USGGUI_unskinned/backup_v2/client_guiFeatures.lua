...
function onClientCharacter(key)
	...
	if not isElement(elementInFocus) or info.guiType ~= "edit" then return end
...
function onClientKey(orgKey,pressed)
	if not isElement(elementInFocus) or not pressed then return end
	local info = GUIinfo[elementInFocus]
	if orgKey == "mouse_wheel_up" or orgKey == "mouse_wheel_down" then	
		if info.guiType == "gridlist" or info.guiType == "treeview" then
			if orgKey == "mouse_wheel_down" then
				local height = info.height
				if info.scrollPixels < height+20 then
					info.scrollPixels = info.scrollPixels+10
				end
			elseif orgKey == "mouse_wheel_up" then
				info.scrollPixels = math.max(0,info.scrollPixels-10)
			end
		end
	end
	...
	if orgKey == "arrow_r" or orgKey == 'arrow_l' then	
		local caret = GUIinfo[elementInFocus].caretIndex
		if caret > 0 and orgKey == "arrow_l" then		
			GUIinfo[elementInFocus].caretIndex = caret - 1			
		elseif caret < #GUIinfo[elementInFocus].text and orgKey == "arrow_r" then		
			GUIinfo[elementInFocus].caretIndex = caret + 1			
		end			
	elseif orgKey == 'backspace' or orgKey == 'delete' then		
		if orgKey == 'backspace' and GUIinfo[elementInFocus].caretIndex >= 1 then		
			local oldText = getText(elementInFocus)
			local firstPart = string.sub(oldText, 0, GUIinfo[elementInFocus].caretIndex-1)
			local lastPart = string.sub(oldText, GUIinfo[elementInFocus].caretIndex+1)	
			GUIinfo[elementInFocus].text = firstPart..lastPart
			GUIinfo[elementInFocus].caretIndex = GUIinfo[elementInFocus].caretIndex-1			
		elseif orgKey == 'delete' and GUIinfo[elementInFocus].caretIndex < #GUIinfo[elementInFocus].text then		
			local oldText = getText(elementInFocus)
			local firstPart = string.sub(oldText, 0, GUIinfo[elementInFocus].caretIndex)
			local lastPart = string.sub(oldText, GUIinfo[elementInFocus].caretIndex+2)	
			GUIinfo[elementInFocus].text = firstPart..lastPart	
			GUIinfo[elementInFocus].caretIndex = GUIinfo[elementInFocus].caretIndex
		end	
	else return		
	end
	
	if GUIinfo[elementInFocus].caretIndex < GUIinfo[elementInFocus].scroll then
		GUIinfo[elementInFocus].scroll = GUIinfo[elementInFocus].scroll-1			
	end
...
function getClickedElement(x,y)
	...
	for elementID=1,#GUIelements do	
		...
		if info.x and info.y then
			...
			if parentInfo and parentInfo.x and parentInfo.y then
	...
	if isElement(element) then
		...
		elseif info.guiType == "gridlist" then 		
			if state == 'down' then
				...
				local x,y = sx-parentX, sy-parentY-info.y-22
