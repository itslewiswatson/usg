...
function drawElementOnRender(element)
	...
	if info.visible and ( not parentInfo or parentInfo.visible ) then
		...
		elseif info.guiType == "editbox" then		
			...
			if elementInFocus == element then
		...
		elseif info.guiType == 'gridlist' then		
			...
			for column=1,#columns do -- draw headers
				...
				dxDrawText(columns[column].text,columnX+5,columnY,columnX+columnWidth,columnY+20,textColor, defaultTextScale*1,"default-bold",'left','center',false,false)
			...
			for row=1,#rows do -- draw rows			
				...
				if info.selectedRow == row and not info.selectedColumn  then 
					dxDrawRectangle(rowX, rowY, rowWidth, rowHeight, tocolor(50,50,100,200)) -- selected row color
				end			
				...
				for i=1,#columns do				
					...
					if rows[row][i] then				
						if info.selectedRow == row and info.selectedColumn == i then						
							dxDrawRectangle(itemX, itemY, itemWidth, itemHeight, tocolor(50,50,100,200)) -- selected row+column color						
						end
						local rowColor = rows[row][i].textColor or defaultTextColor
						if rows[row][i][1] then
							dxDrawText(rows[row][i][1],itemX+5, itemY, itemX+(itemWidth-5), itemY+itemHeight,rowColor, defaultTextScale*1,"default",'left','center',false,false) -- row text
						end					
					end			
			...
			if not drawedgrid then
				dxDrawImage(x,y+22,info.width,info.height-22,info.contentRenderTarget)
			end
		elseif info.guiType == "treeview" then
			local nodes = info.nodes
			dxDrawRectangle(x,y,info.width,info.height, info.color) -- background for nodes
...
function drawElementsOnRender()
	for elementID=1,#GUIelements do
		...
		if not GUIinfo[element].ignoreOnRender then
			drawElementOnRender(element)
		end
	end
end
