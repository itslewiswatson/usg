			...
			if ( maxScroll) then
				if orgKey == "mouse_wheel_down" then
					if ( info.scrollPixels+(0.08*maxScroll) < maxScroll and info.scrollPixels < maxScroll) then
						info.scrollPixels = math.floor(math.min(getElementMaxScroll(elementInFocus),info.scrollPixels+(0.08*maxScroll)))
					end
				elseif orgKey == "mouse_wheel_up" then
					info.scrollPixels = math.max(0,math.floor(info.scrollPixels-(0.08*(maxScroll))))
				end
			end
		end
	end
		...
		elseif info.guiType == "gridlist" then 		
			if state == 'down' then
				...
				if ( loc and height and loc+elY <= sy and loc+elY+height >= sy and sx > elX+info.width-12 ) then
					...
					local rowIndex = math.max(1,math.floor(info.scrollPixels/20)) -- calculate row to start from
					for row=rowIndex,info.rowCount do
...
function manageMouseOnRender()
	...
	and isGUIVisible(elementInFocus) ) then
		...
		if(onEditDisableControls) then guiSetInputMode("no_binds") end
	else
	...
	if ( elementScrollHeld.element ) then
		...
		info.scrollPixels = math.floor(math.min(getElementMaxScroll(elementScrollHeld.element),math.max(0,scrollPos)))
	end
