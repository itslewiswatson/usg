...
function getDimensionsData(element,x,y,width,height,relative)
	...
	if relative then
		...
		width,height = math.max(math.min(width,1),0)*parentWidth,math.max(math.min(height,1),0)*parentHeight			
	end
...
function createWindow(x,y,width,height,relative,text,parent,color)
	if x and y and width and height then		
		...
		resource = sourceResourceRoot, titleBarColor = tocolor(175,75,0,200), textColor = tocolor(0,0,0),ignoreOnRender=ignoreOnRender }
		return element
	end
end
...
function createGridList(x,y,width,height,relative,parent,color)
	if x and y and width and height then
		...
		contentRenderTarget = dxCreateRenderTarget(width,height-21,true), columns = {}, rows = {}, scrollPixels=0, selectionMode=0,ignoreOnRender=ignoreOnRender
		, scrollBarProperties = {}}
		return element
	end
end
...
function createEditBox(x,y,width,height,relative,text,parent,color,clearOnFirstClick,masked,maskedAfterClick)
	if x and y and width and height and text then
		...
		textColor = tocolor(0,0,0,255), ignoreOnRender=ignoreOnRender}
		return element
...
function createMemo(x,y,width,height,relative,text,parent,color)
	if x and y and width and height and text then
		...
		textColor = tocolor(0,0,0,255), ignoreOnRender=ignoreOnRender, lines = {}, scrollPixels = 0, scrollBarProperties = {}}
		return element
	end
end
