...
addEventHandler("onIRCResourceStart",root,
	function ()
		function say (server,channel,user,command,...)
			...
			outputIRC("02* "..ircGetUserNick(user).." on irc: "..message)
		end
		...
		addIRCCommandHandler("!players",
			function (server,channel,user,cmd,room)
				if getPlayerCount() == 0 then
					ircSay(channel,"There are no players ingame.")
				else
					if(room) then
						local players = exports.USGrooms:getPlayersInRoom(room)
						if(players) then
							ircSay(channel,"There are "..#players.." players in the room "..room:upper()..".")
						else
							ircNotice(user, "This room does not exist.")
							return
						end
					else
			...
			function (server,channel,user,command,resName)
				...
				if resource then
					if not startResource(resource) then
						...
						ircNotice(user, "Successfully started "..getResourceName(resource))
					end
				else
			...
			function (server,channel,user,command,resName)
				...
				if resource then
					if not restartResource(resource) then
						...
						ircNotice(user, "Successfully restarted "..getResourceName(resource))						
					end
				else
					ircNotice(user,"Resource '"..resName.."' not found!")
				end
			...
			function (server,channel,user,command,resName)
				...
				if resource then
					if not stopResource(resource) then
						...
						ircNotice(user, "Successfully stopped "..getResourceName(resource))		
					end
				else
					ircNotice(user,"Resource '"..resName.."' not found!")
				end
		...
		function outputCommands (server,channel,user,command)
			...
			for i,cmd in ipairs (ircGetCommands()) do
				if ircIsCommandEchoChannelOnly(cmd) then
					...
					if ircGetCommandLevel(cmd) or 6 <= ircGetUserLevel(user,channel) or 0 then
						table.insert(cmds,cmd)
					end
				end
			end
			ircNotice(user,"You can use these commands: "..table.concat(cmds,", "))
		end
		...
		addIRCCommandHandler("!money",
			function (server,channel,user,command,name)
				...
				local player = getPlayerFromPartialName(name)
				if player then
					ircNotice(user,getPlayerName(player).."'s money: "..exports.USG:formatMoney(getPlayerMoney(player)))
				else
					ircNotice(user,"'"..name.."' no such player")
				end
			end
		...
		addIRCCommandHandler("!health",
			function (server,channel,user,command,name)
				...
				local player = getPlayerFromPartialName(name)
				if player then
					ircNotice(user,getPlayerName(player).."'s health: "..tostring(getElementHealth(player)))
				else
					ircNotice(user,"'"..name.."' no such player")
				end
			end
