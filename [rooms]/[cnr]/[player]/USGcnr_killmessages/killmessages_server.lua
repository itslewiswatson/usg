...
function KillMessages_onPlayerWasted ( totalammo, killer, killerweapon, bodypart )
    if(exports.USGrooms:getPlayerRoom(source) == "dd") then return end
    if(exports.USGrooms:getPlayerRoom(source) == "dm") then return end
...
function outputKillMessage ( killed, wr,wg,wb,killer,kr,kg,kb,weapon,width,resource )
    ...
    if(exports.USGrooms:getPlayerRoom(player) == "cnr") or (exports.USGrooms:getPlayerRoom(player) == "str") or (exports.USGrooms:getPlayerRoom(player) == "tct")then
            local px,py,pz = getElementPosition(player)
            if(getDistanceBetweenPoints2D(x,y,px,py) < 400) then
                triggerClientEvent(player,"onClientPlayerKillMessage",killed,killer,weapon,wr,wg,wb,kr,kg,kb,width,resource )
            end
        end
    end
    return true
