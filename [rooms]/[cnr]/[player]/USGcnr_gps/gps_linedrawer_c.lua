...
function removeLinePoint( name, id )
	if(linePoints[name]) then
		for i, point in ipairs(linePoints[name]) do
			if(point.id == id) then -- tile found
				local loaded = renderStuff[name] ~= nil -- determine if it was loaded before we messed with it
				if(loaded) then -- unload it first
					unloadTile ( name )
				end
				table.remove(linePoints[name],i) -- remove the point
				if(#linePoints[name] == 0) then -- tile empty, destroy it
					linePoints[name] = nil
				elseif(loaded) then -- reload it if not empty
					loadTile( name )
				end
				break
			end
...
function addLinePoint ( posX, posY )
	...
	uniqueID = uniqueID+1
	-- Now add this point
	table.insert ( linePoints[name], { id = uniqueID, posX = tileX, posY = tileY } )
	-- Success!
