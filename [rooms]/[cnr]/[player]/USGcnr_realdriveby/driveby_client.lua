...
local drivebyVehicle

--This function handles the driveby toggling key.
function toggleDriveby()
	--If he's not in a vehicle dont bother
	if not isPedInVehicle( localPlayer ) or exports.USGrooms:getPlayerRoom() ~= "cnr" then return end
	...
	drivebyVehicle = getPedOccupiedVehicle ( localPlayer )
	...
	if equipedWeapon == 0 then
		...
		addEventHandler ( "onClientElementDestroy",drivebyVehicle,toggleDriveby )
		...
		if(isElement(drivebyVehicle)) then
			removeEventHandler ( "onClientElementDestroy",drivebyVehicle,toggleDriveby )
		end
	end
end
...
function removeKeyToggles(vehicle)
	...
	removeEventHandler ( "onClientPlayerVehicleExit",localPlayer,removeKeyToggles )
	if(isElement(drivebyVehicle)) then
		removeEventHandler ( "onClientElementDestroy",drivebyVehicle,removeKeyToggles )
	end
end
