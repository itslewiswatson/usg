...
local DEFAULT_STREAM_DISTANCE = 9999
...
function createCustomBlip ( x, y, width, height, path, fRadius )
	...
	local image = dxImage:create( path, 0, 0, width, height, false )
	...
	setElementData(image.element, "USGblips_visibleDistance", fRadius, false)
	setElementData(image.element, "USGblips_customblipID", fRadius, false)
	VISIBLE_BLIP_COUNT = VISIBLE_BLIP_COUNT + 1
	...
	addEventHandler("onClientElementDestroy", image.element, function () destroyCustomBlip(source) end)
