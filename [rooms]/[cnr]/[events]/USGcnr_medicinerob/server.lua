...
function start()
    ...
    messageCNR("Some medicines need transportation from "..getZoneName(x,y,z).." to SF complex!",255,255,255)
    
    addEventHandler ( "onVehicleStartEnter", helicopter, function(player)
    ...
    addEventHandler ( "onVehicleEnter", helicopter, function(p,seat)
        if(seat == 0)then
        destroyWaypoint()
        ...
        messageCNR("A paramedic entered the transport helicopter and is heading toward SF complex",255,255,255)
            setElementFrozen(helicopter,false)
            driver = p
            addEventHandler ( "onPlayerWasted", driver, onDeathLoose)
            
            addEventHandler ( "onVehicleExit", helicopter, function(p,seat)
                if(seat == 0)then
                    if(isElementWithinMarker(helicopter,waypoint) and isVehicleOnGround ( helicopter ))then
                    messageCNR("The paramedic delivered the medecins to SF complex and is now unloading them!",255,255,255)
                        destroyWaypoint()
                            addEventHandler ( "onVehicleStartEnter", helicopter, function()
                            cancelEvent()
                            end )
                        ...
                        waypointBlip = createBlipAttachedTo ( waypoint, 41, _, _, _,_ ,_, _, _,p )
                            addEventHandler( "onMarkerHit", waypoint, function(p)
                                if(p == driver)then
                                destroyWaypoint()
                                    ...
                                    addEventHandler( "onMarkerHit", waypoint, function(p)
                                        ...
                                        setElementFrozen(p,true)
                                            ...
                                            messageCNR("The medicines have been unloaded and the hangar has been sealed!",255,255,255)
												...
												exports.USGmsg:msg(driver,"You have received "..tostring(drugsPrize).." of each medecin",255,255,255)
                                            end,Time,1)
                                        end
                                    end)
                                end
    ...
    addEventHandler("onVehicleExplode", helicopter, function()
	messageCNR("The helicopter transporting the medicines has blown!",255,255,255)
	loose()
	end)
end
...
waypointBlip = createBlipAttachedTo ( waypoint, 41)
    ...
    addEventHandler( "onMarkerLeave", waypoint, function(p)
        if(exports.USGcnr_jobs:getPlayerJob(p) == "criminal")then
        hidePanel(p)
...
addEventHandler("onPlayerAttemptChangeJob",root,function(jobID)
    if(isElement(colshape))then
