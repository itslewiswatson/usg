...
function loadInteriors()
	clearResourceInteriors(resource)
...
addEventHandler("onPlayerExitRoom", localPlayer,
	function (prevRoom)
		if(prevRoom == "cnr") then
			for resource, _ in pairs(interiors) do
				clearResourceInteriors(resource)
			end
			interiors = {}
		end
	...
	function (res)
		if(getResourceFromName("USGrooms") and getResourceState(getResourceFromName("USGrooms")) == "running" and exports.USGrooms:getPlayerRoom() == "cnr") then
			if(res == resource) then
				...
				interiorLoadElements ( getResourceRootElement(res), res )
				interiorCreateMarkers ( res )
				resourcesPending[res] = true
			end
		elseif(res ~= resource) then
			resourcesPending[res] = true
		end
	end
)

function clearResourceInteriors(resource)
...
addEventHandler("onClientResourceStop", root, clearResourceInteriors)
