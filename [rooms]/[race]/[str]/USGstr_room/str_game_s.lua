...
function spawnPlayerSTR(player)
    ...
    setElementDimension(player, exports.USGrooms:getRoomDimension("str"))
end

function loadGame()
    if(state ~= STATE_FINISHED and state ~= STATE_WAITING) then return false end
    players = exports.USGrooms:getPlayersInRoom("str")
    if(#players == 0) then
        state = STATE_WAITING
        return
    end
    state = STATE_LOADING
    map = exports.USGrace_maps:getRandomMap("str")
    ...
    for i,player in ipairs(players) do
        ...
        outputChatBox("[SHOOTER] Loading map "..(title or "unnamed").." by "..(author or "unknown"), player, 10,170,250)
    end 
    playerMapLoaded = {}
...
function prepareGame()
    ...
    triggerClientEvent(players, "USGstr_room.prepareRound", resourceRoot, vehicles)
...
function endSpawnProtection()
    if(state == STATE_ACTIVE) then
        ...
        for i, player in ipairs(players) do
            local vehicle = getPedOccupiedVehicle(player)
            if(vehicle and exports.USGrooms:getPlayerRoom(player) == "str") then
                table.insert(vehicles, vehicle)
                table.insert(igPlayers, player)
                outputChatBox("[SHOOTER] Spawn protection ended.", player, 10, 170, 250)
            end     
        end
        triggerClientEvent(igPlayers, "USGstr_room.gameStopSpawnProtection", resourceRoot, vehicles)
    end
end
    ...
    if(#players > 1) then
        if(not winner) then
            for i, p in ipairs(players) do
                if(isAlive[p] and exports.USGrooms:getPlayerRoom(p) == "str") then
                    winner = p
                    break
                end
            end
        ...
        if(winner) then
            ...
            exports.USGplayerstats:incrementPlayerStat(winner, "str_wins", 1)           
            outputChatBox("[SHOOTER] You win, you gain $"..reward.."!", winner,0,255,0)
        end
    end
    if(winner) then
        for i, player in ipairs(players) do
            if(isElement(player) and exports.USGrooms:getPlayerRoom(player) == "str") then
                triggerClientEvent(player, "USGstr.roundWon", winner)
            end
        end
    end
...
function endGame()
    for i,player in ipairs(players) do
        if(exports.USGrooms:getPlayerRoom(player) == "str") then
            ...
            triggerClientEvent(player, "USGstr.roundEnd", player)
        end
    end
...
function onPlayerWasted(player)
    ...
    for i, pPlayer in ipairs(players) do
        if(player == pPlayer and isAlive[pPlayer]) then -- he was actually participating
            ...
            if(rank <= 2) then -- there is one player left
                ...
                outputChatBox("[SHOOTER] You finished at #"..rank, player, 200,255,200)
            end 
            index = i
            break
        end
    ...
    if(index and gameStartTick) then -- valid death
        ...
        for i, pPlayer in ipairs(players) do
            if(isElement(pPlayer) and exports.USGrooms:getPlayerRoom(pPlayer) == "str") then
                triggerClientEvent(pPlayer, "USGstr_room.playerWasted", player, rank, deathTime, exports.USG:getPlayerColoredName(player))
            end
        end
    end 
    if(index) then -- find people who are spectating, if they were spectating the person who died, go to the next
        for player, i in pairs(spectators) do
            if(i == index and exports.USGrooms:getPlayerRoom(player) == "str") then
...
addEventHandler("onPlayerWasted", root, 
    function () 
        if(exports.USGrooms:getPlayerRoom(source) == "str") then
...
function setSpectateTarget(player, target, i)
    if(exports.USGrooms:getPlayerRoom(player) ~= "str") then return false end
